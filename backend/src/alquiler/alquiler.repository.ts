...
public async registrarAlquiler(
  garageId: number,
  usuarioId: number,
  ...
  await pool.query(
    'INSERT INTO alquileres (garageId, usuarioId, duracionHoras, servicios, vehiculoId, fechaAlquiler, total) VALUES (?, ?, ?, ?, ?, NOW(), ?)',
    [garageId, usuarioId, duracionHoras, servicios, vehiculoId, total]
  );
}

public async getGarageById(garageId: number): Promise<any> {
  const [rows]: any = await pool.query(
    'SELECT nroGarage, activo, cantLugares FROM garages WHERE nroGarage = ?',
    [garageId]
  );
  return rows[0] ?? null;
}
